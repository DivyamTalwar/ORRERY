...
cleanup() {
  if [ -n "$tmp_dir" ] && [ -d "$tmp_dir" ]; then
    case "$tmp_dir" in
      "$tmp_base"/orrery-verify.*) rm -rf "$tmp_dir" ;;
...
description = "Orrery's complex implementation lane for context-heavy or higher-risk work."
model = "gpt-5.6-terra"
model_reasoning_effort = "max"

developer_instructions = """
You are Orrery's complex implementation worker. Resolve difficult implementation
...
name = "orrery_luna_implementer"
description = "Orrery's routine implementation lane for bounded, fully specified work."
model = "gpt-5.6-luna"
model_reasoning_effort = "max"

developer_instructions = """
You are Orrery's routine implementation worker. Execute the supplied five-part
...
jq empty "$canonical_manifest"
canonical_version=$(jq -r '.version' "$canonical_manifest")
[ -n "$canonical_version" ] && [ "$canonical_version" != null ] || fail "canonical plugin.json has no version"
[ "$(jq -r '.version' "$manifest")" = "$canonical_version" ] || fail "Codex manifest version does not match canonical plugin.json ($canonical_version)"
...
expected = {
    ...
    "orrery-sol-reviewer.toml": {
        "name": "orrery_sol_reviewer",
        "model": "gpt-5.6-sol",
        "model_reasoning_effort": "high",
...
printf '%s\n' \
  ...
  "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_id\",\"parent_thread_id\":\"00000000-0000-7000-8000-000000000000\",\"agent_role\":\"orrery_terra_implementer\",\"agent_path\":\"/root/fixture\",\"model_provider\":\"openai\",\"cwd\":\"/fixture\"}}" \
...
printf '%s\n' "$runtime_output" | jq -e --arg id "$runtime_id" '
  .thread_id == $id and .agent_role == "orrery_terra_implementer"
...
for document in "$skill" "$contracts"; do
  grep -Fq 'agent_type: orrery_terra_implementer' "$document" || fail "missing Terra spawn in $document"
  grep -Fq 'agent_type: orrery_sol_reviewer' "$document" || fail "missing Sol spawn in $document"
