...
A `tools-changed` status is a security stop, not a nuisance: the MCP tool surface no
longer matches the content address the user approved. Report both digests, do not
delegate, and do not re-approve on the user's behalf.
...
Use only saved preferences and observable host capabilities. Codex, Cursor, Claude Code,
VS Code, GitHub Copilot, and Kiro IDE/CLI may invoke the exact native role names
generated by the confirmed adapter. Never translate model names or claim unavailable
effort, cost-tier, or sandbox guarantees. If a configured role is absent or its requested
...
`orrery_routine`, `orrery_high`, and `orrery_advisor`; Cursor, Claude
Code, VS Code, GitHub Copilot, and Kiro use `orrery-routine`, `orrery-high`,
and `orrery-advisor`. Never prescribe Codex identifiers cross-client. The
`render_client_adapter` preview reports the exact `roleIds` for the configured client;
prefer those over any remembered convention. Role files, not per-spawn guesses, carry
client-native model settings. Report routing or sandbox evidence only when exposed by
the host.

The advisor's read-only guarantee differs by host and is reported as
`readOnlyMechanism`. Only `os-sandbox` with an observed `read-only` sandbox policy is
OS-enforced isolation. `tool-allowlist` (Claude Code) genuinely removes edit tools from
the advisor but is not an OS boundary. `frontmatter-flag` and `prompt-only` must be
